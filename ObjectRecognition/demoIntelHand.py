...
try:
    while True:
        ...
        imgRGB = cv2.cvtColor(images, cv2.COLOR_BGR2RGB)
        ...
        fps = 1 / (cTime - pTime)
        pTime = cTime
        cv2.putText(images, str(int(fps)), (18, 70), cv2.FONT_HERSHEY_PLAIN, 3, (255, 0, 255), 3)

        # show webcam
        print(type(images));

        cv2.namedWindow('Align Example', cv2.WINDOW_NORMAL)
        cv2.imshow('Align Rubles Example', images)
        key = cv2.waitKey(1)
